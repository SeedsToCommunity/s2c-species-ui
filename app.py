...
HTML_TAG_PATTERN = re.compile(
    r'<(b|i|strong|em|p|br|ul|li|ol|a|span|div|h[1-6])[^>]*>', re.IGNORECASE)
...
def is_chart_data(data):
    ...
    numeric_count = sum(1 for v in data.values()
...
def parse_json_urls_filter(value):
    ...
    if isinstance(value, dict):
        ...
        if is_tiered_data(value):
            logging.info(
                f"TIERED_DATA: Detected tiered content format (from dict)")
            return {'type': 'tiered_content', 'data': value}
        # Check for similar_species
        if value.get(
                'topic') == 'similar_species' and 'similar_species' in value:
        ...
        url_count = sum(1 for v in value.values()
                        if isinstance(v, str) and str(v).startswith('http'))
    ...
    if value.startswith('http') and not value.startswith(
    ...
    try:
        ...
        if isinstance(parsed, dict):
            logging.info(
                f"PARSE_JSON: Parsed JSON with keys: {list(parsed.keys())[:6]}"
            )
        ...
        if isinstance(parsed, dict):
            ...
            if is_tiered_data(parsed):
                logging.info(
                    f"TIERED_DATA: Detected tiered content format from JSON string"
                )
            ...
            if any(key in parsed for key in ['tier1', 'tier2', 'tier3']):
                logging.warning(
                    f"TIERED_DATA: Found tier keys but is_tiered_data returned False. Keys: {list(parsed.keys())}"
                )
            ...
            if topic == 'similar_species' and has_similar_species_key:
                logging.info(
                    f"SIMILAR_SPECIES: Detected and returning similar_species type"
                )
                return {'type': 'similar_species', 'data': parsed}
            logging.debug(
                f"JSON dict parsed, topic={topic}, has_similar_species_key={has_similar_species_key}, keys={list(parsed.keys())[:5]}"
            )

            # Check if values are image URLs
            image_count = sum(1 for v in parsed.values() if is_image_url(v))
            url_count = sum(1 for v in parsed.values()
        ...
        if isinstance(parsed, list):
            ...
            url_count = sum(1 for v in parsed
                            if isinstance(v, str) and v.startswith('http'))
...
def format_list_content_filter(value):
    ...
    if has_group_tags:
        ...
        for part in parts:
            ...
            if re.match(r'^(?:Native|Non-native|Horticultural|Invasive)\s*—$',
                        part):
                if current_item:
        ...
        if items:
            formatted_items = [
                '• ' + html.escape(item) for item in items if item
            ]
...
def save_column_labels(labels_mapping):
    ...
    try:
        ...
        app.logger.info(
            f"Saved {len(labels_mapping)} column labels to {COLUMN_LABELS_FILE}"
        )
    except Exception as e:
...
def update_column_labels_from_headers(original_columns):
    ...
    for original in original_columns:
        ...
        if original and str(
                original).strip() and not str(original).startswith('Unnamed'):
...
def cleanup_missing_columns(valid_columns):
    ...
    screen_files = [
        'config/screen_identification.json', 'config/screen_collection.json',
        'config/screen_processing.json', 'config/screen_storage.json',
        'config/screen_stratification.json', 'config/screen_planting.json'
    ]
        ...
        try:
            if os.path.exists(screen_file):
                ...
                if len(cleaned_columns) < len(original_columns):
                    ...
                    app.logger.info(
                        f"Cleaned {len(original_columns) - len(cleaned_columns)} missing columns from {screen_file}"
                    )
        except Exception as e:
    ...
    try:
        ...
        if os.path.exists(display_config_file):
            ...
            cleaned_main = [
                col for col in main_cols if col.get('field') in valid_columns
            ]
            ...
            cleaned_filters = [
                col for col in filter_cols if col.get('field') in valid_columns
            ]
            ...
            if modified:
                ...
                app.logger.info(
                    f"Cleaned missing columns from {display_config_file}")
    except Exception as e:
    ...
    try:
        ...
        cleaned_labels = {
            k: v
            for k, v in labels.items() if k in valid_columns
        }
        stale_count = len(labels) - len(cleaned_labels)
        if stale_count > 0:
            save_column_labels(cleaned_labels)
            app.logger.info(
                f"Removed {stale_count} stale entries from column_labels.json")
    except Exception as e:
...
def compute_column_usage(df):
    ...
    screen_configs = {}
    valid_screens = [
        'identification', 'collection', 'processing', 'storage',
        'stratification', 'planting'
    ]
    ...
    for col in all_columns:
        ...
        if any(
                c.get('field') == col
                for c in display_config.get('main_page_columns', [])):
        ...
        usage['sample_data'] = non_empty_values.head(2).tolist() if len(
            non_empty_values) > 0 else []
        usage['fill_percentage'] = round(
            (len(non_empty_values) / len(df) * 100), 1) if len(df) > 0 else 0
    ...
    app.logger.info(
        f"Cached column usage stats for {len(all_columns)} columns")
...
def interpret_conservatism(c_value):
    ...
    if c >= 0 and c <= 1:
        return {
            ...
            "description":
            "This plant grows in all kinds of places, including roadsides and disturbed areas. It's a survivor that isn't picky about conditions."
        }
    elif c >= 2 and c <= 3:
        return {
            "value":
            ...
            "Adaptable",
            "description":
            "This plant can grow in many different settings, including areas that have been somewhat changed by human activity."
        }
    elif c >= 4 and c <= 6:
        return {
            "value":
            ...
            "description":
            "This plant does best in natural areas but can handle some changes to its environment. Finding it suggests the habitat is in decent shape."
        }
    elif c >= 7 and c <= 8:
        return {
            "value":
            c,
            "short":
            "Needs quality habitat",
            "description":
            "This plant is choosy about where it lives. It strongly prefers natural areas that haven't been heavily disturbed."
        }
    elif c >= 9 and c <= 10:
        return {
            "value":
            ...
            "description":
            "This plant only thrives in high-quality natural areas. Finding it is a sign you're in a special place worth protecting."
        }
    return None
...
def interpret_wetland(w_value):
    ...
    if w == -5:
        return {
            ...
            "Loves wet feet",
            "description":
            "This plant almost always grows in wetlands. It thrives standing in water or saturated soil."
        }
    elif w in [-4, -3]:
        return {
            "short":
            ...
            "description":
            "This plant is usually found in wetlands or very moist areas, but can sometimes grow in drier spots."
        }
    elif w in [-2, -1]:
        return {
            "short":
            "Leans toward moist",
            "description":
            "This plant does well with consistent moisture but isn't strictly a wetland species."
        }
    elif w == 0:
        return {
            "short":
            "Flexible about moisture",
            "description":
            "This plant is equally happy in wet or dry spots. It's adaptable with no strong preference."
        }
    elif w in [1, 2]:
        return {
            "short":
            "Leans toward dry",
            "description":
            "This plant prefers drier conditions but can tolerate occasional moisture."
        }
    elif w in [3, 4]:
        return {
            "short":
            ...
            "description":
            "This plant usually grows in well-drained, drier upland areas and may struggle in wet soils."
        }
    elif w == 5:
        return {
            "short":
            "Needs dry ground",
            "description":
            "This plant almost never grows in wetlands. It needs well-drained soil and can rot in soggy conditions."
        }
    else:
        return None
...
def _save_cache_to_disk(df, supplemental_df, file_name,
    ...
    try:
        if os.path.exists(MAIN_CACHE_FILE) and os.path.exists(CACHE_META_FILE):
            ...
            if age_hours < 24:
                ...
                app.logger.info(
                    f"Loaded {len(df)} rows from disk cache (age: {age_hours:.1f}h)"
                )
...
def _get_supplemental_file_info():
    ...
    if _supplemental_file_name:
        col_count = len(_cached_supplemental_data.columns
                        ) if _cached_supplemental_data is not None else 0
        row_count = len(_cached_supplemental_data
                        ) if _cached_supplemental_data is not None else 0
        return {
            'found': True,
    ...
    return {'found': False, 'file_name': None, 'columns': 0, 'rows': 0}
...
def get_google_credentials(write_access=False):
    ...
    if not service_account_json:
        app.logger.warning(
            "GOOGLE_SERVICE_ACCOUNT_JSON secret is not set - Google API features will not work"
        )
        return None
...
def get_google_drive_service_with_write():
    ...
    try:
        ...
        if access_token:
            ...
            service = build('drive',
                            ...
                            cache_discovery=False)
            app.logger.info(
                "Using Replit Google Drive connector for write access")
            return service
    except Exception as e:
    ...
    try:
        ...
        app.logger.error(
            f"Error creating Google Drive service with write access: {str(e)}")
        return None
...
def get_replit_drive_access_token():
    ...
    if repl_identity:
        ...
        app.logger.warning(
            "No Replit token found for connector authentication")
        return None
    ...
    try:
        ...
        if response.status_code != 200:
            app.logger.warning(
                f"Replit connector returned status {response.status_code}")
            return None
        ...
        app.logger.warning(
            "No access token found in Replit connector response")
        return None
...
def find_or_create_folder(parent_folder_id, folder_name):
    ...
    try:
        ...
        app.logger.info(
            f"Created new folder '{folder_name}' with ID: {folder_id}")
        return folder_id, None

    except Exception as e:
        app.logger.error(
            f"Error finding/creating folder '{folder_name}': {str(e)}")
        return None, str(e)
...
def upload_json_to_drive(folder_id, filename, json_data):
    ...
    try:
        ...
        app.logger.info(
            f"Uploaded JSON file '{file_name}' to Drive with ID: {file_id}")
        return {
            'id': file_id,
            'name': file_name,
...
def get_config_folder_id():
    ...
    parent_folder_id = settings.get('data_source',
...
def download_json_from_drive(folder_id, filename):
    ...
    try:
        ...
        app.logger.error(
            f"Error downloading '{filename}' from Drive: {str(e)}")
        return None, str(e)
...
def update_json_in_drive(folder_id, filename, json_data):
    ...
    try:
        ...
        if files:
            ...
            app.logger.info(
                f"Updated config '{filename}' in Drive (ID: {file_id})")
        else:
            # Create new file
            file_metadata = {
                'name': filename,
            ...
            file = service.files().create(
                ...
                fields='id, name, modifiedTime').execute()
            app.logger.info(
                f"Created config '{filename}' in Drive (ID: {file.get('id')})")
...
def load_config_from_drive(config_name):
    ...
    try:
        with open(local_path, 'r') as f:
            config = json.load(f)
        app.logger.info(
            f"Loaded '{filename}' from local file (Drive not available or file not in Drive)"
        )
        return config, 'local'
    except FileNotFoundError:
        app.logger.warning(
            f"Config '{filename}' not found locally or in Drive")
        return None, 'default'
    except Exception as e:
...
def save_config_to_drive(config_name, config_data):
    ...
    columns_key = 'columns' if config_name.startswith(
        'screen_') else 'main_page_columns'
    columns = config_data.get(columns_key, [])

    if len(columns) == 0:
        app.logger.error(
            f"SAFETY: Refusing to save empty config '{filename}' to Drive - this would destroy production data"
        )
    ...
    if result:
        ...
        try:
            ...
            app.logger.warning(
                f"Could not save local backup of {filename}: {e}")
...
def check_google_drive_file_modified(file_id):
    ...
    try:
        ...
        file_metadata = service.files().get(
            fileId=file_id, fields='id, name, modifiedTime').execute()
...
def find_latest_file_in_folder(folder_id, file_prefix):
    ...
    try:
        ...
        if not files:
            app.logger.warning(
                f"No files found in folder {folder_id} with prefix '{file_prefix}'"
            )
        ...
        if _current_file_id != file_id:
            ...
            app.logger.info(
                f"Found new file: '{file_name}' (modified: {modified_time}), was using: {old_file}"
            )
...
def check_for_new_data_file():
    ...
    file_id, file_name, message = find_latest_file_in_folder(
        folder_id, file_prefix)

    if file_id is None:
        return None, message

    # Check if this is a new file
    was_different_file = (_current_file_id
                          != file_id) if _current_file_id else True
...
def check_for_new_supplemental_file():
    ...
    folder_id = settings.get('data_source', {}).get('google_drive_folder_id',
                                                    '')
    supplemental_prefix = settings.get('data_source',
                                       {}).get('supplemental_file_prefix',
                                               'PlantData')
    ...
    try:
        ...
        is_new_file = (previous_file_id
                       != file_id) if previous_file_id else True

        if is_new_file:
            app.logger.info(
                f"Found new supplemental file: '{file_name}' (modified: {modified_time}), was: {_supplemental_file_name or 'None'}"
            )
...
def find_google_sheet_in_folder(folder_id, file_prefix):
    ...
    try:
        ...
        if not files:
            app.logger.info(
                f"No Google Sheets found in folder with prefix '{file_prefix}'"
            )
        ...
        app.logger.info(
            f"Found supplemental Google Sheet: '{file_name}' (modified: {modified_time})"
        )
...
def export_google_sheet_tab_as_csv(file_id, gid):
    ...
    try:
        ...
        result = sheets_service.spreadsheets().values().get(
            spreadsheetId=file_id, range=f"'{target_sheet_name}'").execute()
        ...
        app.logger.info(
            f"Exported {len(values)} rows from sheet '{target_sheet_name}'")
        return csv_content, None

    except Exception as e:
        app.logger.error(
            f"Error exporting Google Sheet tab (gid={gid}): {str(e)}")
        return None, f"Error: {str(e)}"
...
def export_google_sheet_tab_by_name(file_id, tab_name):
    ...
    try:
        ...
        app.logger.info(
            f"Found sheet '{target_sheet_name}' matching requested name '{tab_name}'"
        )
        ...
        app.logger.error(
            f"Error exporting Google Sheet tab by name '{tab_name}': {str(e)}")
        return None, f"Error: {str(e)}"
...
def load_attribution_data(force_reload=False):
    ...
    if _attribution_load_failed_no_creds and not force_reload:
        ...
        if service_account_json:
            app.logger.info(
                "Credentials now available, retrying attribution data load...")
            force_reload = True
            ...
            if os.path.exists(ATTRIBUTION_CACHE_FILE):
                try:
                    ...
                    app.logger.info(
                        "Removed stale attribution disk cache for retry")
                except Exception as e:
                    app.logger.warning(
                        f"Could not remove stale attribution cache: {e}")

    # Return cached data if available and not forcing reload
    if not force_reload and _cached_attribution_data is not None:
        # Validate that cached data is not empty
        if isinstance(_cached_attribution_data,
                      pd.DataFrame) and not _cached_attribution_data.empty:
            return _cached_attribution_data
        # If cached data is empty, clear it and continue to reload
        app.logger.info(
            "Memory cached attribution data is empty, reloading...")
        _cached_attribution_data = None
    ...
    if not force_reload and os.path.exists(ATTRIBUTION_CACHE_FILE):
        try:
            ...
            if isinstance(disk_cache, pd.DataFrame) and not disk_cache.empty:
                ...
                app.logger.info(
                    f"Loaded attribution data from disk cache ({len(disk_cache)} rows)"
                )
                return _cached_attribution_data
            else:
                app.logger.info(
                    "Disk cache attribution data is empty, will reload from source"
                )
        except Exception as e:
            app.logger.warning(
                f"Could not load attribution cache from disk: {e}")
    ...
    attribution_tab_name = settings.get('data_source',
                                        {}).get('attribution_tab_name',
                                                'Column Sources')

    if not folder_id or not supplemental_prefix:
        app.logger.warning(
            "Attribution data not configured - folder_id or supplemental_file_prefix not set in app_settings.json"
        )
        return {}
    ...
    try:
        ...
        if not service_account_json:
            app.logger.warning(
                "Attribution load skipped - GOOGLE_SERVICE_ACCOUNT_JSON not available yet (will retry on next request)"
            )
        ...
        if not file_id:
            file_id, file_name, message = find_google_sheet_in_folder(
                folder_id, supplemental_prefix)

        if not file_id:
            app.logger.warning(
                f"No supplemental data file found for attribution - check Google Drive folder access"
            )
        ...
        csv_content, error = export_google_sheet_tab_by_name(
            file_id, attribution_tab_name)
        if error:
            app.logger.warning(
                f"Could not load attribution tab '{attribution_tab_name}': {error}"
            )
            return {}
        ...
        try:
            ...
            app.logger.warning(
                f"Could not save attribution cache to disk: {e}")
...
def get_attribution_for_field(field_name):
    ...
    if attribution_df is None or (isinstance(attribution_df, pd.DataFrame)
                                  and attribution_df.empty):
        return None
    ...
    for col in df.columns:
        col_values = df[col].astype(str).str.lower().str.replace(
            '_', ' ').str.strip()
        ...
        if not matches.empty:
            ...
            return {
                k: v
                for k, v in row.items() if pd.notna(v) and str(v).strip()
            }
...
def load_prompts_data(force_reload=False):
    ...
    if _prompts_load_failed_no_creds and not force_reload:
        ...
        if service_account_json:
            app.logger.info(
                "Credentials now available, retrying prompts data load...")
            force_reload = True
            _prompts_load_failed_no_creds = False

    # Return cached data if available and not forcing reload
    if not force_reload and _cached_prompts_data is not None:
        if isinstance(_cached_prompts_data,
                      pd.DataFrame) and not _cached_prompts_data.empty:
    ...
    if not force_reload and os.path.exists(PROMPTS_CACHE_FILE):
        try:
            ...
            if isinstance(disk_cache, pd.DataFrame) and not disk_cache.empty:
                ...
                app.logger.info(
                    f"Loaded prompts data from disk cache ({len(disk_cache)} rows)"
                )
                return _cached_prompts_data
            else:
                app.logger.info(
                    "Disk cache prompts data is empty, will reload from source"
                )
        except Exception as e:
    ...
    supplemental_prefix = settings.get('data_source',
                                       {}).get('supplemental_file_prefix',
                                               'PlantData')
    prompts_tab_name = settings.get('data_source',
                                    {}).get('prompts_tab_name', 'Prompts')

    if not folder_id or not supplemental_prefix:
        app.logger.warning(
            "Prompts data not configured - folder_id or supplemental_file_prefix not set"
        )
        return {}
    ...
    try:
        ...
        if not service_account_json:
            app.logger.warning(
                "Prompts load skipped - GOOGLE_SERVICE_ACCOUNT_JSON not available yet"
            )
        ...
        csv_content, error = export_google_sheet_tab_by_name(
            file_id, prompts_tab_name)
        if error:
            app.logger.warning(
                f"Could not load prompts tab '{prompts_tab_name}': {error}")
            return {}
...
def get_prompt_by_type(prompt_type):
    ...
    if prompts_df is None or (isinstance(prompts_df, pd.DataFrame)
                              and prompts_df.empty):
        return ''
    ...
    content_col = prompts_df.columns[1] if len(
        prompts_df.columns) > 1 else None
    for col in prompts_df.columns:
        if col == type_col:
    ...
    for _, row in prompts_df.iterrows():
        row_type = str(row[type_col]).lower().strip() if pd.notna(
            row[type_col]) else ''
...
def load_supplemental_data(force_reload=False):
    ...
    try:
        ...
        app.logger.info(
            f"Successfully loaded {len(df)} rows of supplemental data from '{file_name}'"
        )
...
def merge_supplemental_data(main_df, supplemental_df):
    ...
    try:
        ...
        if supplemental_df.empty:
            ...
            if new_cols:
                app.logger.info(
                    f"Supplemental data has headers but no rows - adding {len(new_cols)} empty columns to preserve assignments"
                )
        ...
        if not supp_has_keys:
            app.logger.warning(
                "Supplemental data missing genus/species columns - cannot merge"
            )
            return main_df
        ...
        if not main_has_keys and not main_has_botanical:
            app.logger.warning(
                "Main data missing genus/species and botanical_name columns - cannot merge supplemental data"
            )
            return main_df
        ...
        if main_has_keys:
            ...
            main_df['_merge_key'] = main_df['botanical_name'].apply(
                create_species_key_from_botanical_name)

        # Create merge key for supplemental data
        supplemental_df['_merge_key'] = supplemental_df.apply(
            create_species_key, axis=1)
        ...
        if not new_cols:
            ...
            main_df['has_community_data'] = main_df['_merge_key'].isin(
                supp_keys)
        ...
        supp_subset = supp_subset.drop_duplicates(subset=['_merge_key'],
                                                  keep='first')

        # Merge
        merged_df = main_df.merge(supp_subset,
                                  on='_merge_key',
                                  how='left',
                                  indicator=True)

        # Add has_community_data flag based on whether merge found a match
        merged_df['has_community_data'] = merged_df['_merge'].apply(
            lambda x: x == 'both')
        ...
        app.logger.info(
            f"Successfully merged {len(new_cols)} supplemental columns: {list(new_cols)}"
        )
        app.logger.info(
            f"Species with community data: {community_count} of {len(merged_df)}"
        )
...
def load_plant_data(force_reload=False, file_id_override=None):
    ...
    with _data_load_lock:
        ...
        # Load settings
        settings = load_app_settings()
        folder_id = settings.get('data_source',
                                 {}).get('google_drive_folder_id', '')
        file_prefix = settings.get('data_source', {}).get('file_prefix', '')
        google_drive_url = settings.get('data_source',
                                        {}).get('google_drive_csv_url', '')
        ...
        try:
            if folder_id and file_prefix:
                ...
                if file_id_override:
                    ...
                    if file_id:
                        app.logger.info(
                            f"Found latest file: {file_name} ({file_id})")
                    else:
                        app.logger.warning(
                            f"Could not find file in folder: {message}")
                        file_id = None
                ...
                if file_id:
                    ...
                    app.logger.info(
                        f"Loading data from Google Drive file ID: {file_id}")
                    ...
                    if len(df) > 0 and 'Unnamed: 0' in df.columns and df.iloc[
                            0, 0] == 'Botanical Name':
                        app.logger.info(
                            "Detected header row in data, fixing column names")
                    ...
                    app.logger.info(
                        f"Successfully loaded {len(df)} rows from Google Drive (folder-based)"
                    )

                    # Load and merge supplemental data
                    supplemental_df = load_supplemental_data(
                    ...
                    if removed:
                        app.logger.warning(
                            f"Removed {len(removed)} missing columns from configs: {removed}"
                        )
                    ...
                    _save_cache_to_disk(df, _cached_supplemental_data,
                                        file_name, _supplemental_file_name)
                    return df
        ...
        try:
            if google_drive_url:
                app.logger.info(
                    f"Loading data from Google Drive URL: {google_drive_url}")
                ...
                app.logger.info(
                    f"Successfully loaded {len(df)} rows from Google Drive (URL-based)"
                )
                ...
                _data_cache_timestamp = pd.Timestamp.now()
                # Save to disk for fast restarts
                _save_cache_to_disk(df, _cached_supplemental_data, 'url_based',
                                    _supplemental_file_name)
                return df
        ...
        try:
            ...
            fallback_files = settings.get('data_source', {}).get(
                'fallback_files', ['origdata.tabsv', 'plants.csv'])
            ...
            app.logger.info(
                f"Successfully loaded {len(df)} rows from local files")
            ...
            # Save to disk for fast restarts
            _save_cache_to_disk(df, _cached_supplemental_data,
                                'local_fallback', _supplemental_file_name)
            return df
...
def load_screen_config(screen_type, force_reload=False):
    ...
    try:
        ...
        app.logger.error(
            f"Screen configuration file for {screen_type} not found")
        return {"screen_name": screen_type.title(), "columns": []}
    except Exception as e:
        app.logger.error(
            f"Error loading screen configuration for {screen_type}: {str(e)}")
        return {"screen_name": screen_type.title(), "columns": []}
...
def filter_plants(df, filters):
    ...
    if filters.get('name'):
        ...
        mask = (filtered_df['botanical_name'].str.lower().str.contains(
            name_query, na=False, regex=False)
                | filtered_df['common_name'].str.lower().str.contains(
                    name_query, na=False, regex=False))
    ...
    if filters.get('start_seed_watch'):
        ...
        mask = filtered_df['start_seed_watch'].str.contains(month_filter,
                                                            na=False,
                                                            regex=False)
        filtered_df = filtered_df[mask]
    ...
    if filters.get('germination_code'):
        ...
        mask = filtered_df['germination_code'].str.contains(germ_filter,
                                                            na=False,
                                                            regex=False)
        filtered_df = filtered_df[mask]
    ...
    if filters.get('light'):
        ...
        mask = filtered_df['light'].str.contains(light_filter,
                                                 na=False,
                                                 regex=False)
        filtered_df = filtered_df[mask]
    ...
    if filters.get('moisture'):
        ...
        mask = filtered_df['moisture'].str.contains(moisture_filter,
                                                    na=False,
                                                    regex=False)
        filtered_df = filtered_df[mask]
    ...
    if filters.get('community_data'):
        if 'has_community_data' in filtered_df.columns:
            filtered_df = filtered_df[filtered_df['has_community_data'] ==
                                      True]
...
def index():
    ...
    try:
        ...
        if df.empty:
            flash(
                'Error: No plant data could be loaded. Please check data files.',
                'error')
        ...
        filters = {
            ...
            'community_data':
            community_data_raw == 'true'  # Boolean for filter logic
        }
        ...
        url_filters = {
            ...
            'community_data':
            'true' if filters['community_data'] else ''  # String for URLs
        }
        ...
        if not hasattr(filtered_df, 'to_dict'):
            app.logger.error(
                f"Filter function returned invalid type: {type(filtered_df)}")
        ...
        app.logger.info(
            f"Successfully loaded {len(plants)} plant species (filtered from {len(df)} total)"
        )
        return render_template('index.html',
                               plants=plants,
                               config=config,
        ...
        if CLOUDINARY_AVAILABLE:
            try:
                ...
                if genus and species_name:
                    cloudinary_images = cloudinary_service.get_species_images(
                        genus, species_name, screen_type)
                    app.logger.debug(
                        f"Cloudinary images for {genus} {species_name}: {len(cloudinary_images)} groups"
                    )
            except Exception as e:
        ...
        try:
            ...
            for attr in all_attributions:
                if attr:
                    ...
                    if first_key:
                        field_name = str(attr.get(first_key,
                                                  '')).lower().replace(
                                                      ' ', '_')
...
def api_species_screen(botanical_name, screen_type):
    ...
    try:
        ...
        try:
            ...
            if isinstance(attribution_df,
                          pd.DataFrame) and not attribution_df.empty:
                for _, row in attribution_df.iterrows():
                    attr = {
                        k: v
...
def api_cloudinary_images(botanical_name):
    ...
    try:
        ...
        if screen_id:
            images = cloudinary_service.get_species_images(
                genus, species_name, screen_id, force_refresh)
        else:
            images = cloudinary_service.get_species_images(
                genus, species_name, force_refresh=force_refresh)
...
def refresh_images_endpoint():
    ...
    try:
        ...
        return jsonify({
            'status': 'refreshed',
            'message':
            'Image cache cleared. Fresh images will be loaded on next species view.',
            'refreshed': True
        })
    except Exception as e:
...
def require_admin_auth(f):
    ...
    def decorated(*args, **kwargs):
        if not check_admin_auth():
            app.logger.info(
                "Admin auth check failed - prompting for credentials")
            return Response('Authentication required to access admin area',
                            401,
...
def admin_dashboard():
    ...
    admin_pages = [{
        ...
        'url': '/admin/review',
        'description':
        'Review and approve community-submitted photos and knowledge',
        'icon': 'check-circle'
    }, {
        'title': 'Issue Reports',
        'url': '/admin/issues',
        'description':
        'View and manage all reported issues from community members',
        'icon': 'alert-triangle'
    }, {
        'title': 'Data Columns',
        'url': '/admin/columns',
        'description':
        'Analyze data columns, usage patterns, and field assignments',
        'icon': 'columns'
    }, {
        'title': 'Column Usage Grid',
        'url': '/admin/column-usage',
        'description':
        'Grid view showing which columns are used on which pages',
        'icon': 'grid'
    }, {
...
def reload_data_api():
    ...
    try:
        ...
        message_parts = [
            f'Loaded {len(df)} plant species from main data file.'
        ]
        ...
        if _cached_supplemental_data is not None and not _cached_supplemental_data.empty:
            ...
            message_parts.append(
                f'Merged {supp_rows} supplemental records ({community_count} species matched).'
            )

        # Reload attribution data (Column Sources tab)
        attribution_df = load_attribution_data(force_reload=True)
        if attribution_df is not None and isinstance(
                attribution_df, pd.DataFrame) and not attribution_df.empty:
            message_parts.append(
                f'Loaded {len(attribution_df)} column source records.')

        # Reload prompts data (Prompts tab)
        prompts_df = load_prompts_data(force_reload=True)
        if prompts_df is not None and isinstance(
                prompts_df, pd.DataFrame) and not prompts_df.empty:
...
def check_for_updates_endpoint():
    ...
    if folder_id and file_prefix:
        ...
        # Check MAIN file
        file_id, file_name, main_message = find_latest_file_in_folder(
            folder_id, file_prefix)
        ...
        if is_new_main_file:
            ...
            app.logger.info(
                f"New supplemental file detected: '{supp_file_name}' - reloading"
            )
            _cached_attribution_data = None
            _cached_prompts_data = None
            ...
            msg_parts = [
                f"New supplemental data loaded: '{supp_file_name}'.",
                f"Main file unchanged: '{file_name}'.",
                f"{len(df)} species loaded."
            ]
            if attribution_df is not None and isinstance(
                    attribution_df, pd.DataFrame) and not attribution_df.empty:
                msg_parts.append(
            ...
            msg_parts = [
                f"Already using latest files. Main: '{file_name}'.",
                f"{len(df) if df is not None else 0} species loaded."
            ]
            if attribution_df is not None and isinstance(
                    attribution_df, pd.DataFrame) and not attribution_df.empty:
                msg_parts.append(
                    f"{len(attribution_df)} column source records.")
            if prompts_df is not None and isinstance(
                    prompts_df, pd.DataFrame) and not prompts_df.empty:
    ...
    if was_modified is None:
        ...
        return jsonify({
            'status': 'reloaded',
            'message':
            f'API not available - forced reload. Loaded {len(df)} species. ({message})',
            'reloaded': True,
            'species_count': len(df),
            'supplemental': supp_info
        })
...
def admin_data_columns():
    ...
    try:
        ...
        for col in all_columns:
            ...
            non_empty_values = df[col].dropna()
            sample_values = non_empty_values.head(3).tolist() if len(
                non_empty_values) > 0 else []
            ...
            columns_info[col] = {
                ...
                'data_type': data_type,
                'used_in': used_in,
                'is_unused': not used_in['main_page']
                and not used_in['screens']
            }
...
def api_data_columns():
    ...
    try:
        ...
        for col in df.columns:
            non_empty_values = df[col].dropna()
            sample_values = non_empty_values.head(5).tolist() if len(
                non_empty_values) > 0 else []

            # Determine data characteristics
            max_length = max(len(str(val)) for val in non_empty_values) if len(
                non_empty_values) > 0 else 0
            data_type = 'text'
            if max_length > 100:
                data_type = 'paragraph'
            elif any(
            ...
            columns_data[col] = {
                ...
                "fill_percentage":
                round((len(non_empty_values) / len(df) *
                       100), 1) if len(df) > 0 else 0,
                "max_length":
                max_length,
                "suggested_type":
                data_type,
        ...
        return {
            ...
            "csv_guidelines": {
                "paragraph_text":
                "Wrap long text in double quotes, use line breaks within quotes",
                "special_characters":
                "Escape internal quotes by doubling them (\"\")",
                "google_sheets":
                "Google Sheets handles CSV formatting automatically"
            }
        }
...
def explain_ai_data():
    ...
    is_development = os.environ.get('REPLIT_ENVIRONMENT') == 'development'
    return render_template('explain_ai_data.html',
                           is_development=is_development)
...
def prompt_explorer():
    ...
    if isinstance(prompts_df, pd.DataFrame) and not prompts_df.empty:
        ...
        app.logger.info(
            f"Prompts: Using type_col='{type_col}', content_col='{content_col}'"
        )
        ...
        for _, row in prompts_df.iterrows():
            ...
            app.logger.debug(
                f"Prompts: row_type='{row_type}', has_content={bool(content)}")
            ...
            if 'base' in row_type or 'shared' in row_type:
                ...
                app.logger.info(
                    f"Prompts: Found base prompt ({len(content)} chars)")
            # Tier-specific guidance (tier 1, tier1, tier 1 guidance, etc.)
                ...
                app.logger.info(
                    f"Prompts: Found tier{current_tier} prompt ({len(content)} chars)"
                )
    ...
    if isinstance(attribution_df, pd.DataFrame) and not attribution_df.empty:
        ...
        for _, row in attribution_df.iterrows():
            field_name = str(row[first_col]).strip() if pd.notna(
                row[first_col]) else ''
            prompt_content = ''
            ...
            if field_name:
                ...
                for prefix, cat_name in category_prefixes:
                    if topic_id.startswith(prefix):
                        category = cat_name
                        display_name = topic_id[len(prefix):].replace(
                            '_', ' ').title()
                        break

                if category == 'General':
                    display_name = field_name.replace('_', ' ').replace(
                        '-', ' ').title()
...
def report_issue(botanical_name):
    ...
    if request.method == 'POST':
        try:
            ...
            flash('Thank you! Your issue report has been submitted.',
                  'success')
            return redirect(
                url_for('species_detail', botanical_name=botanical_name))
...
def admin_column_usage():
    ...
    try:
        ...
        app.logger.debug(
            f"Column usage cache status: {cached is not None}, plant data cache: {_cached_plant_data is not None}"
        )

        if cached is not None:
            app.logger.debug(
                f"Using cached column usage data with {len(cached['data'])} columns"
            )
        ...
        if cached:
            app.logger.debug(
                f"Fresh load successful with {len(cached['data'])} columns")
            return render_template('admin_column_usage.html',
                                   column_usage=cached['data'],
                                   total_columns=cached['total_columns'],
                                   total_species=cached['total_species'],
        ...
        if screen_name == 'main_page':
            ...
            if enabled:
                ...
                if len([c for c in columns
                        if c.get('field') != column_name]) > 0:
                    columns = [
                        c for c in columns if c.get('field') != column_name
                    ]
                else:
                    return {
                        "error": "Cannot remove last column from screen"
                    }, 400
...
def admin_column_reorder():
    ...
    try:
        ...
        valid_screens = [
            'main_page', 'identification', 'collection', 'processing',
            'storage', 'stratification', 'planting'
        ]
        ...
        if screen == 'main_page':
            ...
            columns = screen_config.get('columns', [])
            active_screen_name = screen_config.get('screen_name',
                                                   screen.title())
                ...
                for group_id, group_data in all_groups.items():
                    if screen in group_data.get('default_screens', []):
                        screen_image_groups.append({
                            ...
                            'icon':
                            group_data.get('icon', 'image'),
                            'description':
                            group_data.get('description', '')
                        })
            except Exception as e:
                app.logger.error(
                    f"Error loading image groups for reorder page: {e}")
...
_upload_rate_limit = {
}  # IP -> {'minute': (timestamp, count), 'hour': (timestamp, count)}
    ...
    try:
        ...
        for file in files:
            ...
            if not allowed_file(file.filename):
                error_count += 1
                app.logger.warning(
                    f"Skipped invalid file type: {file.filename}")
                continue
            ...
            result = cloudinary_service.upload_user_image(file_data=file,
                                                          genus=genus,
                                                          species=species,
                                                          username=username,
                                                          tags=tags)
            ...
            if result.get('success'):
                success_count += 1
                app.logger.info(
                    f"User image uploaded: {result.get('public_id')} by {username}"
                )
            else:
                error_count += 1
                app.logger.error(
                    f"User image upload failed: {result.get('error')}")

        # Flash appropriate message
        if success_count > 0 and error_count == 0:
            flash(f'Thank you! {success_count} image(s) submitted for review.',
                  'success')
        elif success_count > 0 and error_count > 0:
            flash(
                f'{success_count} image(s) submitted, {error_count} failed (invalid type or too large).',
                'warning')
        else:
            flash('All uploads failed. Please check file types and sizes.',
                  'error')
...
def save_pending_metadata(botanical_name, text, categories, submitter_name):
    ...
    safe_name = re.sub(r'[^a-zA-Z0-9_]', '',
                       submitter_name.replace(' ', '_').lower())
    if not safe_name:
        safe_name = 'anonymous'
    ...
    for filename in os.listdir(PENDING_METADATA_DIR):
        if filename.endswith('.json'):
            ...
            try:
                ...
                app.logger.error(
                    f"Error reading metadata file {filename}: {e}")
...
def submit_contribution(botanical_name):
    ...
    try:
        ...
        if metadata_text:
            ...
            try:
                save_pending_metadata(botanical_name, metadata_text,
                                      categories, submitter_name)
                metadata_saved = True
                app.logger.info(
                    f"Metadata submitted for {botanical_name} by {submitter_name}"
                )
            except Exception as e:
                metadata_error = True
        ...
        if messages:
            flash(f"Thank you! Submitted {' and '.join(messages)} for review.",
                  'success')

        # Report any errors
        if image_errors > 0 and image_count == 0:
            flash('Image uploads failed. Please check file types and sizes.',
                  'error')
        elif image_errors > 0:
        ...
        if metadata_error:
            flash(
                'Failed to save your knowledge submission. Please try again.',
                'error')
    ...
    try:
        ...
        if CLOUDINARY_AVAILABLE:
            ...
            pending_images = [
                ...
                if img['public_id'] not in processed_ids
            ]
            ...
            if pending_images:
                ...
                if pending_image.get('genus') and pending_image.get('species'):
                    approved_images = cloudinary_service.get_approved_images_for_species(
                        pending_image['genus'], pending_image['species'])
            else:
...
def admin_approve_metadata():
    ...
    try:
        ...
        tier1_folder_id, error = find_or_create_folder(parent_folder_id,
                                                       'Tier 1 Sources')
        ...
        flash(f'Knowledge submission approved and uploaded to Google Drive',
              'success')
        app.logger.info(
            f"Metadata approved and uploaded: {filename} -> Drive ID: {result.get('id')}"
        )
